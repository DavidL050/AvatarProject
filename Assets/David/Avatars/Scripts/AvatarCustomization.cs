...
// SOLUCIÓN: Reemplaza el método UpdateFacialHair_Internal() en tu archivo AvatarCustomization.cs

private void UpdateFacialHair_Internal() {
    if (CurrentGender != AvatarGender.Male) {
        // Si cambiamos a femenino y hay facial hair, lo eliminamos
    ...
    UFacialHair[] existingFacialHairs = CurrentBase.GetComponentsInChildren<UFacialHair>();

    // Validar que existan facial hair items
    if (AvatarReferences.FacialHairItems == null || AvatarReferences.FacialHairItems.Length == 0) {
        Debug.LogWarning("[AvatarCustomization] No facial hair items available in AvatarReferences");
        foreach (UFacialHair facialHair in existingFacialHairs) {
            SafeDestroyGameObject_Internal(facialHair.gameObject);
        }
        _facialHairObject = null;
        return;
    ...
    // Verificar que el facial hair item tiene un mesh asignado
    if (AvatarReferences.FacialHairItems[index] == null || 
        AvatarReferences.FacialHairItems[index].FacialHairmesh == null) {
        Debug.LogError($"[AvatarCustomization] FacialHairItem at index {index} or its mesh is null!");
        return;
    }
    
    int materialIndex = AvatarReferences.FacialHairItems[index].HasVariations() ? 
        Mathf.Clamp(FacialHairMaterialIndex, 0, AvatarReferences.FacialHairItems[index].Variations.Length - 1) : 0;
    
    // Buscar facial hair existente
    _facialHairObject = null;
    foreach (UFacialHair facialHair in existingFacialHairs) {
        ...
        // Eliminar facial hairs que no coinciden
        SafeDestroyGameObject_Internal(facialHair.gameObject);
    }

    // Si no existe, crear nuevo facial hair
    if (_facialHairObject == null) {
        Debug.Log($"[AvatarCustomization] Creating new facial hair: {AvatarReferences.FacialHairItems[index].name}");
        
        // CORRECCIÓN CRÍTICA: Instanciar el GameObject del SkinnedMeshRenderer, no el componente
        GameObject facialHairMeshObject = AvatarReferences.FacialHairItems[index].FacialHairmesh.gameObject;
        _facialHairObject = Instantiate(facialHairMeshObject);
        
        // Configurar el padre
        _facialHairObject.transform.SetParent(CurrentBase.transform, false);
        _facialHairObject.transform.localPosition = Vector3.zero;
        _facialHairObject.transform.localRotation = Quaternion.identity;
        _facialHairObject.transform.localScale = Vector3.one;
        
        // Añadir componente UFacialHair
        UFacialHair facialHairComponent = _facialHairObject.GetComponent<UFacialHair>();
        if (facialHairComponent == null) {
            facialHairComponent = _facialHairObject.AddComponent<UFacialHair>();
        }
        facialHairComponent.FacialHairItem = AvatarReferences.FacialHairItems[index];
        
        // Obtener el SkinnedMeshRenderer
        SkinnedMeshRenderer renderer = _facialHairObject.GetComponent<SkinnedMeshRenderer>();
        if (renderer == null) {
            Debug.LogError($"[AvatarCustomization] SkinnedMeshRenderer not found on facial hair object!");
            SafeDestroyGameObject_Internal(_facialHairObject);
            _facialHairObject = null;
            return;
        }
        
        // Asignar los bones del cuerpo
        renderer.bones = _bodyBones;
        renderer.rootBone = CurrentGenderSkinnedRenderer.rootBone;
        
        // Asignar el material
        if (AvatarReferences.FacialHairItems[index].HasVariations()) {
            renderer.sharedMaterial = AvatarReferences.FacialHairItems[index].Variations[materialIndex];
        }
        
        Debug.Log($"[AvatarCustomization] Facial hair created successfully. Active: {_facialHairObject.activeSelf}");
    }

    // Actualizar el material si el facial hair ya existe
    if (_facialHairObject != null) {
        SkinnedMeshRenderer renderer = _facialHairObject.GetComponent<SkinnedMeshRenderer>();
        if (renderer != null && AvatarReferences.FacialHairItems[index].HasVariations()) {
            renderer.sharedMaterial = AvatarReferences.FacialHairItems[index].Variations[materialIndex];
            Debug.Log($"[AvatarCustomization] Updated facial hair material to variation {materialIndex}");
        }
        
        // Asegurarse de que el GameObject esté activo
        if (!_facialHairObject.activeSelf) {
            _facialHairObject.SetActive(true);
            Debug.Log("[AvatarCustomization] Facial hair was inactive, activating it now");
        }
    }

    // Actualizar las blend shapes del facial hair
    UpdateFacialHair_Shape_Internal();
}
